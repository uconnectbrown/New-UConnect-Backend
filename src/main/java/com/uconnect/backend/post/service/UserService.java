package com.uconnect.backend.post.service;
...
import com.uconnect.backend.post.dao.UserDAO;
...
public class UserService implements UserDetailsService {
    ...
    @Override
    public Post loadUserByUsername(String username) {
        try {
     ...
     *
     * @param post A user object to populate the remainder of the user's data
     * @return An exit code
     */
    public int createNewUser(Post post) {
        try {
            ...
            log.error("Unexpected exception while creating new user " + post.getUsername() + ": {}", e);
            return -2;
        }
    }
    ...
    public String generateTraditionalJWT(String username) {
        ...
        final Post post = loadUserByUsername(username);

        return jwtUtility.generateToken(post);
    }
    ...
    public String generateOAuthJWT(String username) {
        ...
        try {
            post = loadUserByUsername(username);
        ...
        if (!UserCreationType.O_AUTH.equals(post.getCreationType())) {
